import { l as _export_sfc, s as _sfc_main$E } from '../server.mjs';
import { mergeProps, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent } from 'vue/server-renderer';
import '../../nitro/node-server.mjs';
import 'node:http';
import 'node:https';
import 'fs';
import 'path';
import 'node:fs';
import 'node:url';
import 'shiki-es';
import 'consola/core';
import 'unified';
import 'mdast-util-to-string';
import 'micromark';
import 'unist-util-stringify-position';
import 'micromark-util-character';
import 'micromark-util-chunked';
import 'micromark-util-resolve-all';
import 'micromark-util-sanitize-uri';
import 'slugify';
import 'remark-parse';
import 'remark-rehype';
import 'remark-mdc';
import 'hast-util-to-string';
import 'github-slugger';
import 'detab';
import 'remark-emoji';
import 'remark-gfm';
import 'rehype-external-links';
import 'rehype-sort-attribute-values';
import 'rehype-sort-attributes';
import 'rehype-raw';
import 'unist-util-visit';
import 'ipx';
import 'unhead';
import '@unhead/shared';
import 'vue-router';
import '@iconify/vue/dist/offline';
import '@iconify/vue';
import '@vee-validate/zod';
import 'vee-validate';
import 'zod';
import '@headlessui/vue';
import '@vue/shared';
import 'chroma-js';
import 'tailwindcss/colors.js';

const _sfc_main = {};
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  const _component_BaseAvatar = _sfc_main$E;
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-wrap items-end gap-4" }, _attrs))}>`);
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-muted-200 dark:bg-muted-600 text-muted-800 dark:text-muted-300"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-primary-100 dark:bg-primary-500 text-primary-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-lime-100 text-lime-500 dark:bg-lime-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-green-100 text-green-500 dark:bg-green-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-emerald-100 text-emerald-500 dark:bg-emerald-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-teal-100 text-teal-500 dark:bg-teal-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-cyan-100 text-cyan-500 dark:bg-cyan-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-sky-100 text-sky-500 dark:bg-sky-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-blue-100 text-blue-500 dark:bg-blue-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-indigo-100 text-indigo-500 dark:bg-indigo-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-violet-100 text-violet-500 dark:bg-violet-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-purple-100 text-purple-500 dark:bg-purple-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-fuchsia-100 text-fuchsia-500 dark:bg-fuchsia-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-pink-100 text-pink-500 dark:bg-pink-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-rose-100 text-rose-500 dark:bg-rose-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-red-100 text-red-500 dark:bg-red-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-orange-100 text-orange-500 dark:bg-orange-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-amber-100 text-amber-500 dark:bg-amber-500 dark:text-white"
  }, null, _parent));
  _push(ssrRenderComponent(_component_BaseAvatar, {
    shape: "full",
    text: "CT",
    size: "md",
    class: "bg-yellow-100 text-yellow-500 dark:bg-yellow-500 dark:text-white"
  }, null, _parent));
  _push(`</div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("../layers/documentation/examples/avatar/circle-fake-colors.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const circleFakeColors = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);

export { circleFakeColors as default };
//# sourceMappingURL=circle-fake-colors-6c9a9637.mjs.map
